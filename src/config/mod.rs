...
/// The default configuration template for OS.
#[cfg(target_os = "linux")]
const DEFAULT_OS_CONFIG: &str = include_template!("config/os/linux.toml");
#[cfg(target_os = "macos")]
const DEFAULT_OS_CONFIG: &str = include_template!("config/os/macos.toml");
#[cfg(target_os = "windows")]
const DEFAULT_OS_CONFIG: &str = include_template!("config/os/windows.toml");
...
impl Config {
    pub fn new() -> anyhow::Result<Self> {
        let sources = vec![
            ...
            ::config::File::from_str(DEFAULT_OS_CONFIG, ::config::FileFormat::Toml),
        ];
